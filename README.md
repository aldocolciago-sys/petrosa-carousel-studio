...
- **Finale a loop** (attivo di default): il video si chiude sfumando dall'ultima slide alla prima, cosi' quando Instagram/TikTok lo rimandano in automatico non si vede lo stacco. Aggiunge circa mezzo secondo, si puo' disattivare.
- **Reel breve indipendente** ("solo hook"): usa solo la prima slide (l'hook) per pochi secondi, con una caption propria diversa da quella del post, che invita a scoprire il resto sul profilo. Pensato per la scoperta (For You / Esplora), da affiancare ogni tanto al Reel completo del post.
- Scheda **Audio & sync**: si segnano una volta per brano i punti in cui vengono cantati i versi (bottone "Qui"). I punti stanno nel browser; scarica `audio-sync.json` e mettilo in `data/` per averli ovunque. I testi in `data/songs.txt` riportano ogni ripetizione di un ritornello per intero (come nel songbook), non una volta sola: conta anche per la sincronizzazione, che si basa sulla posizione nel testo.
...
- **Piano settimanale** (colonna a sinistra): un pulsante ("Crea piano") genera 5 o 7 caroselli, ognuno con il suo Reel. Argomenti alternati (brano, recensione, membro, live, album, band), mood diversi, lunghezze diverse, nessun brano/caption/CTA/copertina ripetuti.
- Per ogni giorno del calendario: "Apri nell'editor" per rivederlo e modificarlo a mano; **"Crea Reel"** registra il video di quel giorno senza dover aprire l'editor (resta comunque un passo manuale, un giorno alla volta: tieni la scheda aperta e in primo piano, ci vuole circa un minuto); **"Programma carosello"** e **"Programma Reel"** li inviano a PostFast separatamente, cosi' puoi programmare prima il carosello e il Reel con calma quando il video e' pronto (o solo uno dei due). Gli account su cui pubblicare si scelgono una volta sola, sopra la lista dei giorni ("Carica account PostFast"): puoi selezionarne piu' di uno, anche su piattaforme diverse insieme (es. Instagram + TikTok + Facebook).
- "Scarica tutto il piano (ZIP)" resta disponibile per la pubblicazione manuale: crea una cartella per giorno con carosello, Reel, caption e istruzioni piu' un `PIANO.txt` con il calendario. "Programma tutto su PostFast" invia in un colpo solo tutti i caroselli e i Reel della settimana (comodo quando li hai gia' rivisti tutti).
- **👍/👎 per giorno**: segna com'e' andato un post gia' pubblicato (a occhio, o con le statistiche di Instagram/TikTok). Resta solo nel browser: i mood con piu' 👍 compaiono in un promemoria sopra il calendario, e i contenuti segnati piu' spesso 👎 vengono scelti un po' meno spesso nei prossimi piani (mai esclusi del tutto).
...
## PostFast: piattaforme collegabili
PostFast (`postfa.st`) puo' collegare Facebook, Instagram, Threads, X (Twitter), LinkedIn, TikTok, YouTube, Pinterest, BlueSky, Telegram e Google Business Profile. L'app mostra automaticamente tutti gli account che colleghi su PostFast (non solo Instagram/TikTok, che restano pre-selezionati per default perche' sono il pubblico principale dei Petrosa): li trovi in "Carica account PostFast" nel piano settimanale, o "Carica account collegati" nell'editor del singolo post.

## Libreria: contenuti che spingono condivisioni/commenti, riga SEO
Oltre alle ricette gia' presenti, la libreria (`data/library.json`) include hook "a confronto" pensati per far commentare (es. "Stoner or doom? Pick one."), CTA e caption con un invito esplicito a condividere/taggare un amico, e un pool di frasi SEO (`seoLines`) con le parole chiave che le persone cercano davvero (genere, anno, citta', etichetta): una viene aggiunta in coda alla caption, coerente col mood, senza duplicarla se e' gia' presente nel testo. TikTok e Instagram leggono anche il testo della caption, non solo gli hashtag.

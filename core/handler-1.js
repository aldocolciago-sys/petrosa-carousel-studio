...
async function relayBlobVideoToPostfast(pathname, contentType) {
  ...
  // L'URL firmato di PostFast e' un upload S3-compatibile: se il PUT arriva in chunked transfer-encoding (il
  // comportamento di default di fetch/undici quando il body e' uno stream) lo rifiuta con "411 MissingContentLength".
  // Vercel Blob restituisce sempre il Content-Length sulla GET (la dimensione del blob e' nota), quindi lo
  // giriamo esplicitamente sulla PUT: passato a mano, undici lo rispetta e manda il corpo senza chunking
  // (verificato: gli stessi byte, nessun header Transfer-Encoding).
  const length = src.headers.get('content-length');
  if (!length) throw new Error('Impossibile determinare la dimensione del video su Vercel Blob (Content-Length mancante): riprova a caricarlo.');
  const put = await fetch(pf.signedUrl, { method: 'PUT', headers: { 'content-type': type, 'content-length': length }, body: src.body, duplex: 'half' });
  if (!put.ok) {

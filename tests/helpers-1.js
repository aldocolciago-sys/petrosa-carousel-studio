...
async function startMockPostfast() {
  ...
  const srv = http.createServer(async (req, res) => {
    ...
    if (u.pathname.startsWith('/put/')) {
      // PostFast vero e' un endpoint S3-compatibile: rifiuta un PUT senza Content-Length esplicito (arrivato in
      // chunked transfer-encoding) con "411 MissingContentLength" - lo riproduciamo qui, altrimenti questo mock
      // non avrebbe mai potuto far emergere il bug del relay che mandava il video in chunked (vedi handler.js,
      // relayBlobVideoToPostfast).
      if (!req.headers['content-length']) {
        res.writeHead(411, { 'content-type': 'application/xml', ...cors });
        return res.end('<?xml version="1.0" encoding="UTF-8"?><Error><Code>MissingContentLength</Code><Message>You must provide the Content-Length HTTP header.</Message></Error>');
      }
      state.puts.push({ key: u.pathname.slice(5), size: raw.length, type: req.headers['content-type'] });
